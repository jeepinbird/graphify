...
def _always_on(basename: str) -> str:
    ...
    The CLAUDE.md always-on block lives as committed markdown next to this
    module. The installer injects it verbatim via ``_replace_or_append_section``.
    """
...
def _packaged_skill_refs_dir(platform_name: str) -> Path | None:
    ...
    ``graphify/skills/<bundle>/references/``.

    If the bundle directory ``graphify/skills/<bundle>/`` is not present in this
    build, this returns None and the host installs the monolithic SKILL.md with
    no references/ sidecar. When the bundle directory IS present but lacks its
...
def install(platform: str = "claude", *, project: bool = False, project_dir: Path | None = None) -> None:
    _print_banner()
    # "windows" is an alias kept for the Windows default path; both map to the
    # single Claude skill destination.
    if platform == "windows":
        platform = "claude"
    if platform not in _PLATFORM_CONFIG:
        print(
            f"error: unknown platform '{platform}'. Only 'claude' is supported.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Refresh version stamps in all previously-installed skill dirs so
    # stale-version warnings don't fire for scopes not explicitly re-installed.
    if project:
...
def _print_install_usage() -> None:
    print("Usage: graphify install [--project]")
    print("Installs the graphify skill into Claude Code (.claude/skills/).")


# The always-on instruction block is packaged markdown under graphify/always_on/
# (claude-md.md), injected verbatim into CLAUDE.md by the installer.

_CLAUDE_MD_MARKER = "## graphify"


def _project_install(platform_name: str, project_dir: Path | None = None) -> None:
    """Install the graphify skill/config files in the current project (Claude Code)."""
    project_dir = project_dir or Path(".")
...
def _project_uninstall(platform_name: str, project_dir: Path | None = None) -> None:
    """Remove project-scoped graphify skill/config files only (Claude Code)."""
    project_dir = project_dir or Path(".")
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    """Remove the graphify Claude Code skill, hook, and git hooks from the project."""
    pd = project_dir or Path(".")
    print("Uninstalling graphify...\n")

    # Skill-file / config-section uninstaller (Claude Code).
    claude_uninstall(pd)
...
def claude_uninstall(project_dir: Path | None = None, *, project: bool = False) -> None:
    ...
    The bare `graphify uninstall` and `graphify claude uninstall` must remove the
    installed skill, not just strip CLAUDE.md, or the progressive-disclosure tree
    (SKILL.md + references/) is orphaned (#1121).
    """
    project_dir = project_dir or Path(".")
...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("  uninstall               remove the graphify skill, CLAUDE.md section, and hooks")
    ...
    if cmd == "install":
        ...
        for arg in sys.argv[2:]:
            ...
            if arg == "--project":
                ...
                print(f"error: unknown install argument '{arg}'", file=sys.stderr)
                sys.exit(1)
        if project_scope:
