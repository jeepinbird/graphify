...
Turn a folder of Markdown/knowledge-base notes and SQL schemas into a navigable knowledge graph with community detection, an honest audit trail, and three outputs: interactive HTML, GraphRAG-ready JSON, and a plain-language GRAPH_REPORT.md.
...
- If `total_words` > 2,000,000 OR `total_files` > 500: show the warning. Then compute the top 5 first-level subdirectories by file count:
  ...
  - Concatenate all file lists across all types (`code`, `document`).
...
Load files from `graphify-out/.graphify_uncached.txt`. Split into chunks of 20-25 files each. When splitting, group files from the same directory together so related notes land in the same chunk and cross-file relationships are more likely to be extracted.
...
Before running any subcommand below (`--update`, `--cluster-only`, `query`, `path`, `explain`, `--watch`), check that `.graphify_python` exists. If it's missing (e.g. user deleted `graphify-out/`), re-resolve the interpreter first:
...
Not part of the default build. When the user passes `--watch` to auto-rebuild the graph on file changes, see `references/watch.md`.
